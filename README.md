...
Учтем, что нет необходимости записывать, что какой-то символ повторился 0 раз. 
Если длина серии будет отрицательное число, то его модуль будет означать сколько символов далее в последовательности встречаются по одному разу.
А если длина серии является неотрицательным числом `Х`, то оно означает, что следующий символ встретился в последовательности `X+2` раз.

Тогда сжатая строка запишется в виде `-9ABCABCABC1D4E` и будет занимать 14 байт (представление в памяти в 16-ричном виде: `F7 41 42 43 41 42 43 41 42 43 01 44 04 45`).

Как же быть, если длина серии равна 130 символам и более? 
В этом случае серию разделяют на части так, чтобы длина части не превышала 129 символов. 
Например, серия, состоящая из 200 символов «A», будет закодирована следующей строкой: `7F 41 45 41`.
Пояснение: `200 = 129 + 71`, `129 - 2 = 127` в 16-ричном виде `7F`, `71 - 2 = 69` в 16-ричном виде `45`.

Как быть, если неповторяющихся символов более 128? В этом случае серию разделяют на части так, чтобы длина части не превышала 128 символов. 
